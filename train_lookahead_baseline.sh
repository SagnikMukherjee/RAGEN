#!/usr/bin/env bash
# Experiment 1, Arm A (baseline): existing <think>+<answer> format.
# Train Sokoban-4 nonlinear, eval OOD on Sokoban-4 (in-dist) + Sokoban-5 + Sokoban-6 nonlinear.
# Mirrors ablate_single_turn.sh. Differences:
#   - val tags include 5step and 6step OOD (instead of linear/nonlinear dual)
...
# === Shared config (matches ablate_single_turn.sh) ===
...
MAX_TOKENS=400
TRAIN_TYPE=mixed
EXP_NAME=lookahead-A-baseline-sokoban-${GRID}x${GRID}-${SOLVE_STEPS}step-${TURNS}turn-${ACTIONS_PER_TURN}act-${TRAIN_TYPE}-${MODEL//\//_}

echo "=== Arm A (baseline, think+answer): train ${SOLVE_STEPS}step ${TRAIN_TYPE}, eval 4/5/6step ==="
